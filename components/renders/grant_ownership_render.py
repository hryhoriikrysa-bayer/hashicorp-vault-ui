...
def render_grant_ownership_header():
    title = "Grant ownership"
    description = "Grant ownership of the secret path for specific user with form provided below."

    return render_header(title, description)


def render_grant_ownership_component():
    full_html = f"""
    <!DOCTYPE html>
    <html>
    <head>
